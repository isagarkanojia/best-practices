package com.software.practices.solutions.java8.map;
...
public class MapSnippets {
    ...
    public static void main(String[] args) {
        ...
        getStringJoinedNames(people);

        getListOfNames(people);

    }

    private static List<String> getListOfNames(List<Person> people) {
        return people.stream().map(p -> p.getName()).collect(Collectors.toList());
    }

    private static String getStringJoinedNames(List<Person> people) {
